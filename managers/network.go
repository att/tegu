...
type Network struct {						// defines a network
	...
	ip2mac		map[string]*string			// IP to mac
	ip2vmid		map[string]*string			// ip to vm-id translation
	vmid2phost	map[string]*string			// vmid to physical host name
	vmid2ip		map[string]*string			// vmid to ip address
}
...
/*
	tegu-lite
	Using the various vm2 ip2 maps, build the host array as though it came from floodlight.
*/
func (n *Network) build_hlist( ) ( hlist []gizmos.FL_host_json ) {
	var (
		sep string = ""
		jstr string					// we'll build a json string and then parse it
	)
	jstr = "[ "
	hlist = make( []gizmos.FL_host_json, 4096 )

	i := 0
	if n != nil {								// first time round n will be nil
		for ip, mac := range n.ip2mac {
			...
			if vmid != nil {						// skip if we don't find a vmid
				...
				jstr += fmt.Sprintf( `%s{ "mac": [ %q ], "ipv4":, [ %q ], "attachmentpoint": [ { "switchdpid": %q, "port": -128 } ] }`, sep, *mac, ip, swname )
				sep = ", "
	...
	hlist = hlist[0:i]
return
	err := json.Unmarshal( []byte( jstr ), &hlist )			// unpack the json into the host list
	if err != nil {
		net_sheep.Baa( 0, "WRN: FL_hosts: error during json unpack: %s", err )
		hlist = nil;
		return
	}
	return
...
func build( old_net *Network, flhost *string, max_capacity int64 ) (n *Network) {
	...
	if strings.Index( *flhost, ":" ) >= 0  {
		...
		if err != nil {
			net_sheep.Baa( 0, "WRN: unable to read static links from %s: %s", *flhost, err )
			return
		}
	}
...
net_sheep.Baa( 1, "hlist has %d elements", len( hlist ) )
net_sheep.Baa( 1, "linkgs has %d elements", len( links ) )
...
func Network_mgr( nch chan *ipc.Chmsg, sdn_host *string ) {
	...
	if act_net == nil {
		net_sheep.Baa( 0, "WRN: initial build of network failed" )
	} else {
		...
		select {					// assume we might have multiple channels in future
			case req = <- nch:
				...
				switch req.Msg_type {
					...
					case REQ_VMID2PHOST:									// tegu-light
						if req.Req_data != nil {
