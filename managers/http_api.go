						...
						if ntokens > 1 {
							...
							if ntokens > 2 {
								...
								switch( tokens[2] ) {
									...
									case "sfs":
										sfs_logger.Set_level( nv )

									default:
										state = "ERROR"
										http_sheep.Baa( 1, "unrecognised subsystem name given with verbose level: %s", tokens[2] )
										jreason = fmt.Sprintf( `"unrecognised subsystem name given; must be one of: agent, osif, resmgr, http, fqmgr, sfs or net"` )
								}
...
func Http_api( api_port *string, nwch chan *ipc.Chmsg, rmch chan *ipc.Chmsg ) {
	...
	if cfg_data["sfs"] != nil {
		enable_sfs = true
		if p := cfg_data["sfs"]["enable"]; p != nil {
			p2 := strings.ToLower(*p)
			enable_sfs = (p2 == "true") || (p2 == "yes")
		}
	}
	...
	if enable_sfs {
		http.HandleFunc( "/tegu/chain/", chain_api_handler )
		http.HandleFunc( "/tegu/fc/",    fc_api_handler )
		http.HandleFunc( "/tegu/group/", group_api_handler )
		http_sheep.Baa( 1, "scalable flow steering URLs are ENABLED" )
	} else {
		http_sheep.Baa( 1, "scalable flow steering is disabled" )
	}
