...
#				18 May 2015 - Now reqiures the presence of an enabler file in /etc/tegu to actually set
#					the queues. If invoked without the file or -f option it will clear all queues.
# ----------------------------------------------------------------------------------------------------------
#
...
if (( ! force ))			# CAUTION:  with the elimination of HTB queues this script is functionally dead.
then						#			We simulate an empty data file to clean up (remove) all queues.
	if (( delete_data )) && [[ -n $1 && -f $1 ]]		# ensure the data file is deleted if needed
	then
		rm -f $1
	fi

	echo "HTB queues have been disabled, queues not needed and thus not created"

	data_file=/tmp/PID$$.dummy_data
	touch $data_file
	delete_data=0
	gen_q1=0				# clear the br-rl best effort queue
else						# if force option set, then set up queues per the data
