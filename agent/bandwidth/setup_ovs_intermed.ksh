...
#				set the metadata value to 1 before resubmitteing to table 0. Setting metadata
#				prevents the matched rule, or any other flow-mod that the agent creates from 
#				matching. (we used to use alternate tables, 90-99, for this; we only need one
#				meta value now, so we can just hard set it in the matched flow-mod.
#				If setting flow-mods on intermediate bridges is not enabled, then these flow-mods
#				are cleared.
#
...
#				non-reservation traffic will be set to 0. This flow-mod (referred to as the 
#				priority-10 flow-mod) should be the lowest priority flow-mod that the agent
#				sets to ensure that it matches last. It will also only match if metadata
#				is a hard value of 0, so that any other agent added flow-mod can prevent 
#				the resettting of a DSCP value by setting metadata.
#
...
function usage
{
	...
	version 1.2/1c164
	usage: $argv0 [-B | -b bride(s)] [-d difserv] [-D] [-e max-tput] [-I] [-l log-file] [-m min] [-n] [-T] [-v] [-x exclude-bridge-list]
...
#
#  set up best effort and priority queues for marked traffic on 'intermediate' bridges (br-tun)
#  etc.   This shouldn't be used because OVS no longer connectes br-int to these bridges with 
#  a veth and so there is no queue to manage.  It's broken out to keep it (we've reverted enought
#  times with rate limiting that I'm unwilling to remove it completely), but to make it easy to 
#  turn off/not use. 
function setup_intermed
{

	typeset fmod_data=/tmp/PID$$.fdata
	typeset rl_data=/tmp/PID$$.rdata
	typeset queue_data=/tmp/PID$$.qdata
	...
	then
		...
		do
			...
			do
				...
				send_ovs_fmod $noexec  -t 0  --match -m 0/1 -T $dscp --action -M 1 -q 1  -R ",0" -N  add 0xbeef $b			# set queue and meta, then resub to table 0 to pick up other fmods
				lrc=0
				rc=$(( lrc + $? ))
				# deprecated -- we no longer use alternate tables to set meta data values
		...
		do
			...
			do
				...
				send_ovs_fmod $noexec  -t 0  --match -m 0/1 -T $dscp --action -q 1 -M 1  -R ",0" -N  del 0xbeef $b			# set queue and meta flag, then resub
				lrc=0
				rc=$(( lrc + $? ))
...
function setup_promotion
{
	sudo ovs-vsctl show | awk -v sudo="$sudo" '
		BEGIN {
			snarf = 0;
			pidx = 0
		}
	...
	then
		...
		then
			...
			$forreal $cmd		# no exec mode; just echo things
		fi
	else
		if (( verbose ))
		then
...
thost=$(hostname)		# target host (our host)
...
then
	...
	ql_setup_irl $noexec -D 	# delete rate limiting bridges and connections
fi

# --- funciton call to setup intermediate bridges with besteffort/priority queues removed

setup_promotion				# set gre ports to promote the dscp markings to the header of tunnel packets
...
then
	...
	then
		# CAUTION:  the meta value match is a _hard_ value of zero, not a mask match 
		if ! send_ovs_fmod  $noexec -t 0 -p 10 --match  -m 0x00 --action -T 0  -M 1  -R ",0" -N add 0xfeed br-int  # turn off dscp, submit for meta mark, then resubmit to 0
		then
