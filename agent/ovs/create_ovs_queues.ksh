...
function usage
{
	...
	version 2.2/15064
	usage: $argv0 [-d] [-e entry-max-rate] [-h host] [-k] [-l brname] [-n] [-s] [-v] tegu-data-filename
	...
	-s enable static queue 0 sizing.
...
log_file=""
static_q0size=0			# -s enables; causes q0 to be statically sized rather than variable based on other queue sizes
...
then
	...
	then
		...
		#echo "first parameter ($1) is not a file (invoking original script)  [FAIL]" 
		#create_ovs_blanket_queues "$@"		# deprecated -- we don't support it 
			...
			for( i = 1; i <= n; i++ )					# set up individual queues for queue 0 needs based on switch/port q0max values  (only if q0 was not supplied in the data
			{
				...
				if( !saw_queue0[s,p] )					#if queue 0 was not set in the input data for this switch/port pair
				{
					...
					if( q0iq[mvs] == "" )
					{
						...
						if( static_q0size )
							printf( " -- --id=@iq%d create Queue other-config:min-rate=1000 other-config:priority=%d %s\n", iqid, 1000, burst_string );
						else
